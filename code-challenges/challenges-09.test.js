...
function transformToLis(obj){
  ...
  for (let index=0;index< objectdata.length ;index++){
    newArr.push(`<li>${objectdata[index][0]}: ${objectdata[index][1]}</li>`);
  }
  return newArr;
}
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let numberOfElements=arr.reduce((acc,val)=>{
    acc=acc+1;
    return acc;
  },0);
...
const reversedString = (str) => {
  ...
  let revStr= newArr.reduce((acc,val,idx)=>{
    return acc+=newArr[newArr.length-1 - idx];
